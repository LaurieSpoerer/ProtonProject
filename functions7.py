...
def experiment_pvdr_known_ROI(data_dir, dark_dir, row_middle=600, plus_minus=50):
    imgs = getarray(data_dir)[0]
    darks = getarray(dark_dir)[0]
    dark_mean = np.mean(darks,axis=0)
    del darks
...
def experiment():
    ...
    while not choice_1_q:
        choice_1_a = (input("Would you like to see the PVDR, type {pvdr}, PVDR with known ROI, type {pvdrkr}, calibration, type {cal} , or multi dark calibration, type {multi_cal}?: "))
        if choice_1_a == 'pvdr':
            experiment_pvdr(img_dir,dark_dir)
            choice_1_q = True
